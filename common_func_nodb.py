...
def Gemini_Trans(Text,lang_sy='ar',Api_Index=0):
...
ترجم هذا النص بأكمله بدقة إلى {F_L}  👇
  ...
  try : 
    ...
    print('Gemini Trans ',err)
    New_Index = Api_Index+1 
    if New_Index < len(Gemini_Apis):
      return Gemini_Trans(Text,lang_sy,New_Index)
    else :
      raise ValueError('انتهت توكنات اليوم 🌿')
      
def Gemini_Trans_Txt(TxtFile,lang_sy='ar'):
  Txt_File = TxtFile.replace('.txt','_Translated.txt')
  Check_File(Txt_File)
  Text = open(TxtFile,'r').read()
  Gemini_CTxt(TxtFile,Txt_File,Text,lang_sy,0,10000)
  return Txt_File
  
def Gemini_CTxt(TxtFile,Txt_File,Text,lang_sy,Req_Count=0,Limit=20000):
  rest = ''
  with open(Txt_File,'a') as f : 
    if len(Text) > Limit : 
      Textlist = Wrap_Text(Text,Limit)
      for Num,part in enumerate(Textlist) : 
        ...
        if Res_Text == 'ERROR' :
          ...
          if New_Limit != 0 :
            return Gemini_CTxt(TxtFile,Txt_File,Text,lang_sy,Req_Count,New_Limit)
          else : 
           Rest_File = Txt_File.replace('.txt','_Rest.txt')
...
########### AI TRANSCRIPTION ###############
Gemini_Apis = Gemini_Tokens

def Gemini_Transcribe(Media_File,Req_Count=0,Api_Index=0):
 TxtFile = ('.' if Media_File[0]=='.' else '' ) + Media_File.split('.')[1 if Media_File[0]=='.' else 0 ] + '_GTranscribed.txt'
 client = genai.Client(api_key=Gemini_Apis[Api_Index])
 Transcribe_Prompt = f"""
قم بتفريغ هذا الملف الصوتي بأكمله بدقة بدون ذكر الطوابع الزمنية 
  """ 
 Media_File = Mp3_Conv(Media_File)
 try : 
    file = client.files.upload(file=Media_File)
    response = client.models.generate_content(model=Gemini_Model, contents=[Transcribe_Prompt, file])
    Res = response.text
    open(TxtFile,'a').write(Res)
    Req_Count += 1
    return TxtFile
 except Exception as err : 
    Req_Count+=1
    ...
    if New_Index < len(Gemini_Apis):
      if Req_Count%15 == 0 :
        time.sleep(60)
      return Gemini_Transcribe(Media_File,Req_Count,New_Index)
    else :
      raise ValueError('انتهت توكنات اليوم 🌿')
...
def upld2arch(upldarchpath,Msg):
  ...
  Cancel_Option = f"{Arch_Url}"
  Cancel_Op = ['حذف','DelArch']
  Cancel_BUTTON = [[InlineKeyboardButton(Cancel_Op[0],callback_data=f"{Cancel_Op[1]}_Fill")]]
  Replied = Msg_Reply(Msg,Cancel_Option,Cancel_BUTTON)
 

def delarch_file(file_name):
  Arch_Del_Cmd = f'''rclone deletefile myarchive:{bucketname}/"{file_name}"'''
  try :
    subprocess.check_output(Arch_Del_Cmd,shell=True,stderr=subprocess.STDOUT,text=True)
  except subprocess.CalledProcessError as e  :
    time.sleep(180)
    return delarch_file(file_name)
...
def Trans_Func(mp3file,ispublic):
  ...
  private_ar = "NCCGCWTO7ILBXGIEEYSLQF2SBVQDZK5X"
